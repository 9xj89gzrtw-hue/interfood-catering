#!/usr/bin/env python3
"""
Компиляция промпта v2.2 для Lovable под кейтеринг Nilov Catering (СПб).
Версия: v2.2 (Lovable Edition, 24 июня 2026)

ОСНОВНЫЕ ИЗМЕНЕНИЯ vs v2.1 (на основе аудита 20 веб-поисков):
1. КРИТИЧНО: Vercel Edge Functions УСТАРЕЛИ — переведены на Vercel Functions (Node.js runtime)
2. ДОБАВЛЕНО: WCAG 2.2 AA compliance (9 новых критериев vs 2.1)
3. ДОБАВЛЕНО: Core Web Vitals 2026 thresholds (LCP<2.5s, INP<200ms, CLS<0.1)
4. ДОБАВЛЕНО: 2026 catering trends — interactive food stations, sustainable menus, experiential dining
5. ДОБАВЛЕНО: Food photography art direction — dark & moody, hands-in-shot, imperfect plating, monthly refresh
6. ДОБАВЛЕНО: Premium storytelling framework (editorial narrative)
7. ДОБАВЛЕНО: Trust signals 25-element checklist (2026 research)
8. ДОБАВЛЕНО: B2B event planner referral strategy
9. ДОБАВЛЕНО: Monthly content refresh strategy (hero dishes refresh)
10. ДОБАВЛЕНО: AI-powered micro-animations & adaptive interactions
11. ДОБАВЛЕНО: Liquid/scroll-driven animations (GSAP-style)
12. УТОЧНЕНО: shadcn@canary для Tailwind v4 + React 19
13. ДОБАВЛЕНО: Schema.org расширен — 35% higher CTR с structured data
14. ДОБАВЛЕНО: Premium color psychology — "signal, not shout"
"""
from pathlib import Path

OUTPUT = Path("/home/z/my-project/download/lovable_prompt_nilov_catering_v2_2.md")
OUTPUT.parent.mkdir(parents=True, exist_ok=True)

PROMPT = r"""# ПРОМПТ ДЛЯ LOVABLE — САЙТ NILOV CATERING (СПб) · v2.2

> **Версия:** 2.2 (Lovable Edition, Awwwards-grade) · **Дата:** 24 июня 2026
...
- What US consumers want from restaurants 2026 (McKinsey): https://www.mckinsey.com/industries/retail/our-insights/what-us-consumers-want-from-restaurants-in-2
- Global Luxury Food Market (CAGR 17.45%): https://www.custommarketinsights.com/report/luxury-food-market

### Trust signals
- 25 Website Trust Signals 2026: https://emporionsoft.com/website-trust-signals-checklist
...
try:
    ...
    print(f"📊 Размер: {chars:,} символов, {lines:,} строк")
    print(f"🎯 Токенов (cl100k_base): {tokens:,}")
    print(f"💾 Размер файла: {OUTPUT.stat().st_size / 1024:.1f} KB")
except ImportError:
    print(f"✅ Файл сохранён: {OUTPUT}")
    print(f"📊 Размер: {len(PROMPT):,} символов, {PROMPT.count(chr(10)) + 1:,} строк")
    print(f"💾 Размер файла: {OUTPUT.stat().st_size / 1024:.1f} KB")
