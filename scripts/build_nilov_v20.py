#!/usr/bin/env python3
"""
Build nilov_catering_v20.html — works in iMessage Quick Look (NO JavaScript!)

KEY INSIGHT: iMessage Quick Look STRIPS all <script> tags but CSS animations
and images WORK. So we design for NO JS first, then JS enhances.

v20 fixes:
1. Hero image: opacity:1 by default (visible in Quick Look), JS fades it in
2. Hero animations: CSS @keyframes run WITHOUT .js prefix (work in Quick Look)
3. Counter values: real numbers (12, 850, 98) in HTML, JS animates from 0
4. All images visible by default — no JS dependency for visibility
5. .js class only used for progressive enhancement, not basic visibility
6. Calculator: static price table always visible, interactive calc hidden without JS
7. FAQ: answers visible without JS, accordion is enhancement only
8. No <noscript> dependency — everything works without it
"""
...
OUTPUT = '/home/z/my-project/download/nilov_catering_v20.html'
...
/* HERO IMAGE: visible by default (works in iMessage Quick Look without JS) */
.hero__bg img{width:100%;height:100%;-o-object-fit:cover;object-fit:cover;-webkit-transform:scale(1.1);transform:scale(1.1);-webkit-transition:opacity 1s ease;transition:opacity 1s ease;opacity:1}
/* With JS: hide image until loaded, then fade in */
.js .hero__bg img{opacity:0}
.hero__bg img.loaded{opacity:1!important}
...
/* HERO ANIMATIONS: CSS @keyframes work in iMessage Quick Look! */
...
.hero__subtitle{font-size:1rem;font-size:clamp(1rem,2vw,1.3rem);font-weight:300;letter-spacing:0.2em;text-transform:uppercase;margin-bottom:2.5rem;opacity:0;-webkit-animation:heroFadeUp 1s 0.6s forwards;animation:heroFadeUp 1s 0.6s forwards}
...
/* SELECT: native appearance for iOS WKWebView picker */
...
/* Interactive calculator — hidden without JS since it can't work */
.js .calc__interactive{display:block}
.calc__interactive{display:none}
/* Static price table — visible by default, hidden when JS works */
...
/* FAQ answers: VISIBLE by default (works in Quick Look), hidden only with .js class */
...
/* ===== REVEAL ANIMATION ===== */
/* Default: VISIBLE. Only hidden when .js class is set (JS will reveal on scroll) */
...
<section class="section section--dark" id="calculator">
  <div class="container">
    <div class="calc">
      ...
      <!-- Interactive calculator — only works with JS -->
      <div class="calc__interactive rv">
      ...
      <!-- Static price table — always visible, hidden when JS works -->
      <div class="calc__static rv">
        ...
        <p style="color:#F5F0E8;font-size:0.85rem;margin-top:1.5rem;text-align:center">Для точного расчёта свяжитесь с нами</p>
        <div style="text-align:center;margin-top:1rem"><a href="#contact" class="btn btn--gold">Связаться</a></div>
      </div>
    </div>
  </div>
</section>
...
(function(){
  ...
  /* Also calc on select blur (fallback for WKWebView) */
