#!/usr/bin/env python3
"""
Build nilov_catering_v18.html — world-class catering website
Fully self-contained, Safari + Telegram WebView + iMessage WebView safe

ALL FIXES from deep audit:
- NO background-attachment:fixed (broken in iOS WebView)
- NO IntersectionObserver (not in all WebViews)
- NO ES6+ (no const/let/arrow functions/template literals)
- ALL -webkit- prefixes
- viewport-fit=cover + env(safe-area-inset-*)
- requestAnimationFrame with fallback
- scroll event + getBoundingClientRect for reveal/counter animations
- All images as base64 data URIs
- inset:0 replaced with top/right/bottom/left (Safari <14.1)
- clamp() with fallback values (Safari <13.1)
- Flex gap with margin fallbacks (Safari <14.1)
- <noscript> block for no-JS environments
- mailto:/tel: with copyable text fallback
- target="_blank" removed (blocked in iMessage)
- z-index 10000+ for Telegram header conflict
- iOS scroll lock (position:fixed on body)
- Single scroll listener (merged)
- @media(hover:hover) for hover states
- input type fixes for mobile
"""
...
OUTPUT = '/home/z/my-project/download/nilov_catering_v18.html'
...
.btn{display:inline-block;padding:1rem 2.5rem;font-family:'Raleway',sans-serif;font-weight:600;font-size:0.85rem;letter-spacing:0.15em;text-transform:uppercase;-webkit-transition:all 0.3s ease;transition:all 0.3s ease;position:relative;overflow:hidden}
...
.nav__link{color:#F5F0E8;font-size:0.78rem;font-weight:500;letter-spacing:0.12em;text-transform:uppercase;-webkit-transition:color 0.3s ease;transition:color 0.3s ease;position:relative;margin-left:2rem}
...
.nav__burger{display:none;-webkit-flex-direction:column;flex-direction:column;padding:8px;cursor:pointer}
...
.mobile-menu__link{color:#F5F0E8;font-size:1.4rem;font-family:'Cormorant Garamond',serif;padding:1rem 0;letter-spacing:0.1em;-webkit-transition:color 0.3s ease;transition:color 0.3s ease}
...
.hero__title{font-size:2.8rem;font-size:clamp(2.8rem,7vw,5.5rem);font-weight:700;margin-bottom:1.5rem;opacity:0;-webkit-transform:translateY(30px);transform:translateY(30px);-webkit-animation:heroFadeUp 1s 0.3s forwards;animation:heroFadeUp 1s 0.3s forwards}
.hero__subtitle{font-size:1rem;font-size:clamp(1rem,2vw,1.3rem);font-weight:300;letter-spacing:0.2em;text-transform:uppercase;opacity:0;-webkit-animation:heroFadeUp 1s 0.6s forwards;animation:heroFadeUp 1s 0.6s forwards;margin-bottom:2.5rem}
.hero__cta{opacity:0;-webkit-animation:heroFadeUp 1s 0.9s forwards;animation:heroFadeUp 1s 0.9s forwards}
.hero__scroll{position:absolute;bottom:2rem;left:50%;-webkit-transform:translateX(-50%);transform:translateX(-50%);z-index:2;color:#F5F0E8;font-size:0.7rem;letter-spacing:0.2em;text-transform:uppercase;text-align:center;opacity:0;-webkit-animation:heroFadeUp 1s 1.2s forwards;animation:heroFadeUp 1s 1.2s forwards}
...
.format-card{position:relative;overflow:hidden;cursor:pointer}
...
.gallery__item{-webkit-column-break-inside:avoid;break-inside:avoid;margin-bottom:1rem;overflow:hidden;cursor:pointer;position:relative}
...
.calc__select,.calc__input{background:rgba(255,255,255,0.05);border:1px solid rgba(200,169,110,0.3);color:#F5F0E8;padding:0.9rem 1rem;font-size:0.9rem;-webkit-appearance:none;appearance:none;-webkit-transition:border-color 0.3s ease;transition:border-color 0.3s ease;outline:none}
...
.faq-item__a{max-height:0;overflow:hidden;-webkit-transition:max-height 0.4s ease;transition:max-height 0.4s ease;font-size:0.95rem;line-height:1.8;color:#666;padding:0}
...
.contact__input,.contact__textarea{background:rgba(255,255,255,0.05);border:1px solid rgba(200,169,110,0.3);color:#F5F0E8;padding:1rem;font-size:0.9rem;-webkit-transition:border-color 0.3s ease;transition:border-color 0.3s ease;outline:none;width:100%;margin-bottom:1.2rem}
...
.lightbox__close{position:absolute;top:1.5rem;right:1.5rem;color:#F5F0E8;font-size:2rem;cursor:pointer;width:44px;height:44px;display:-webkit-flex;display:flex;-webkit-justify-content:center;justify-content:center;-webkit-align-items:center;align-items:center}

/* ===== WHATSAPP FLOAT ===== */
.wa-float{position:fixed;bottom:2rem;right:2rem;z-index:10001;width:56px;height:56px;border-radius:50%;background:#25D366;display:-webkit-flex;display:flex;-webkit-justify-content:center;justify-content:center;-webkit-align-items:center;align-items:center;-webkit-box-shadow:0 4px 20px rgba(37,211,102,0.4);box-shadow:0 4px 20px rgba(37,211,102,0.4);-webkit-transition:-webkit-transform 0.3s ease;transition:transform 0.3s ease}
.wa-float svg{width:28px;height:28px;fill:#fff}

/* ===== REVEAL ANIMATION ===== */
.rv,.rv-left,.rv-right,.rv-scale,.rv-clip{opacity:0;-webkit-transition:opacity 0.8s ease,-webkit-transform 0.8s ease;transition:opacity 0.8s ease,transform 0.8s ease}
.rv{-webkit-transform:translateY(40px);transform:translateY(40px)}
.rv-left{-webkit-transform:translateX(-40px);transform:translateX(-40px)}
.rv-right{-webkit-transform:translateX(40px);transform:translateX(40px)}
.rv-scale{-webkit-transform:scale(0.9);transform:scale(0.9)}
.rv-clip{-webkit-clip-path:inset(0 100% 0 0);clip-path:inset(0 100% 0 0)}
...
<!-- NOSCRIPT: visibility fallback for environments without JS -->
<noscript>
<style>
.rv,.rv-left,.rv-right,.rv-scale,.rv-clip{opacity:1!important;-webkit-transform:none!important;transform:none!important;-webkit-clip-path:none!important;clip-path:none!important}
.hero__title,.hero__subtitle,.hero__cta,.hero__scroll{opacity:1!important;-webkit-transform:none!important;transform:none!important}
.faq-item__a{max-height:none!important;padding:0 0 1.2rem!important}
</style>
</noscript>
</head>
<body>
...
<nav class="nav" id="nav">
  <div class="nav__inner">
    ...
    <div class="nav__links">
      ...
      <a href="#gallery" class="nav__link">Портфолио</a>
...
<div class="mobile-menu" id="mobileMenu">
  ...
  <a href="#gallery" class="mobile-menu__link">Портфолио</a>
...
<section class="hero" id="hero">
  <div class="hero__bg">
    <img src="''' + hero_img + '''" alt="Catering by Nilov" id="heroImg">
  </div>
  <div class="hero__overlay"></div>
  <div class="hero__content">
    <h1 class="hero__title"><span class="ln"><span class="ln-i">Кейтеринг</span></span>
    <span class="ln"><span class="ln-i">в стиле</span></span>
    <span class="ln"><span class="ln-i" style="color:#C8A96E">Nilov</span></span></h1>
  ...
  <div class="container">
    <div class="philosophy__grid">
      <div class="philosophy__text rv-left">
        <h2>Готовим как <em>в лучшем ресторане</em>, обслуживаем как <em>в частном клубе</em></h2>
        <p>Nilov Catering — это не просто еда на вынос. Это гастрономический опыт, созданный шеф-поваром Дмитрием Ниловым лично для вашего события. Каждое блюдо готовится из свежайших продуктов, которые мы отбираем вручную на фермерских хозяйствах Ленинградской области и у проверенных поставщиков из Европы и Азии.</p>
        <p>Мы не работаем по шаблонам. Каждое меню — это результат диалога с клиентом, понимания концепции события и профессионального видения шефа. Фуршет на 30 человек или свадьба на 500 — мы подходим к каждому проекту с одинаковым вниманием к деталям.</p>
        <div class="philosophy__stat">
          <div class="philosophy__stat-item rv">
            <div class="philosophy__stat-num" data-count="12">0</div>
            <div class="philosophy__stat-label">лет опыта</div>
          </div>
          <div class="philosophy__stat-item rv">
            <div class="philosophy__stat-num" data-count="850">0</div>
            <div class="philosophy__stat-label">мероприятий</div>
          </div>
          <div class="philosophy__stat-item rv">
            <div class="philosophy__stat-num" data-count="98">0</div>
            <div class="philosophy__stat-label">% довольных</div>
          </div>
        </div>
      </div>
      <div class="philosophy__img rv-right">
        <img src="''' + about_img + '''" alt="Nilov Catering philosophy">
      </div>
    </div>
  </div>
</section>
...
<section class="section" id="formats">
  <div class="container">
    <h2 class="rv" style="text-align:center">Подберём <em>идеальный</em> формат</h2>
    <p class="rv" style="text-align:center;color:#666;max-width:600px;margin:0 auto">Выберите тип мероприятия, и мы составим меню, подберём персонал и оборудование</p>
    <div class="formats__grid">
      <div class="format-card rv">
        ...
        <div class="format-card__content">
          ...
          <div class="format-card__desc">Изысканные канапе и закуски для свободного общения гостей</div>
      ...
      <div class="format-card rv">
        ...
        <div class="format-card__content">
          ...
          <div class="format-card__desc">Полноценный ужин с обслуживанием и индивидуальным меню</div>
...
<!-- WEDDING -->
<section class="section section--dark wedding" id="wedding">
  <div class="wedding__bg"><img src="''' + wedding_img + '''" alt="Wedding catering"></div>
  <div class="wedding__overlay"></div>
  <div class="container">
    <div class="wedding__content rv-scale">
      <div class="wedding__badge">Специальное предложение</div>
      <h2>Свадьба мечты с <em>флористикой в подарок</em></h2>
      <p style="font-size:1.05rem;line-height:1.8;opacity:0.9;margin-top:1rem">Организуем свадебное застолье, которое запомнится каждому гостю. Авторское меню, премиальное обслуживание и внимание к каждой детали — от подачи блюд до оформления зала.</p>
      <div class="wedding__features">
        <div class="wedding__feature">
          <div class="wedding__feature-icon">Venue</div>
          <div>Подбор площадки</div>
        </div>
        <div class="wedding__feature">
          <div class="wedding__feature-icon">Menu</div>
          <div>Авторское меню</div>
        </div>
        <div class="wedding__feature">
          <div class="wedding__feature-icon">Bar</div>
          <div>Коктейль-бар</div>
        </div>
        <div class="wedding__feature">
          <div class="wedding__feature-icon">Decor</div>
          <div>Флористика в подарок</div>
        </div>
      </div>
      <a href="#contact" class="btn btn--light" style="margin-top:1.5rem">Обсудить свадьбу</a>
    </div>
  </div>
</section>
...
<section class="section section--dark reviews" id="reviews">
  ...
  <div class="container reviews__content">
    <h2 class="rv" style="text-align:center">Что говорят <em>наши клиенты</em></h2>
    <div class="reviews__list">
      <div class="review rv">
        <div class="review__stars">&#9733; &#9733; &#9733; &#9733; &#9733;</div>
        <div class="review__text">Невероятный уровень сервиса и вкуса! Гости до сих пор вспоминают наш свадебный ужин. Каждое блюдо было произведением искусства, а обслуживание — безупречным. Дмитрий лично контролировал всё от начала до конца.</div>
        <div class="review__author">Анна и Алексей, свадьба на 120 гостей</div>
      </div>
      <div class="review rv">
        <div class="review__stars">&#9733; &#9733; &#9733; &#9733; &#9733;</div>
        <div class="review__text">Заказывали фуршет на корпоративный юбилей — 200 человек. Всё прошло идеально: красивая подача, разнообразные закуски, внимательный персонал. Коллеги были в восторге, а руководство уже планирует следующее мероприятие с Nilov.</div>
        <div class="review__author">Марина К., HR-директор TechCorp</div>
      </div>
      <div class="review rv">
        <div class="review__stars">&#9733; &#9733; &#9733; &#9733; &#9733;</div>
        <div class="review__text">Третий год сотрудничаем с Nilov Catering на ежегодной конференции. Стабильно высокий уровень, креативное меню и безупречная логистика. Рекомендую всем, кто ценит качество и надёжность.</div>
        <div class="review__author">Дмитрий С., организатор конференций</div>
      </div>
    </div>
  </div>
</section>
  ...
  <div class="container">
    <div class="about__grid">
      <div class="about__photo rv-left">
        <img src="''' + about_img + '''" alt="Дмитрий Нилов">
      </div>
      <div class="about__text rv-right">
        <h2>Дмитрий Нилов</h2>
        <p>Основатель и шеф-повар Nilov Catering. Более 12 лет в профессиональной кухне, обучение у мастеров в Италии и Франции, работа в ведущих ресторанах Санкт-Петербурга. Дмитрий создал компанию, объединяющую ресторанный подход к качеству с гибкостью кейтеринговой службы.</p>
        <p>Каждое меню — это отражение его философии: только свежие продукты, честные техники и уважение к ингредиенту. Он лично дегустирует каждое блюдо перед выездом на мероприятие и ручается за качество.</p>
        <div class="about__quote">«Еда — это эмоция. Я хочу, чтобы каждый гость на вашем мероприятии почувствовал радость от того, что ест.»</div>
        <a href="#contact" class="btn btn--outline" style="margin-top:1rem">Связаться с шефом</a>
      </div>
    </div>
  </div>
</section>
...
<section class="section section--dark" id="gallery">
  <div class="container">
    <h2 class="rv" style="text-align:center">Наши <em>блюда</em></h2>
    <p class="rv" style="text-align:center;color:#999;max-width:500px;margin:0 auto">Каждое блюдо — маленький шедевр, созданный нашей командой</p>
    <div class="gallery__grid">
      <div class="gallery__item rv" data-lightbox><img src="''' + gallery_imgs[0] + '''" alt="Gallery 1"></div>
      <div class="gallery__item rv" data-lightbox><img src="''' + gallery_imgs[1] + '''" alt="Gallery 2"></div>
      <div class="gallery__item rv" data-lightbox><img src="''' + gallery_imgs[2] + '''" alt="Gallery 3"></div>
      <div class="gallery__item rv" data-lightbox><img src="''' + gallery_imgs[3] + '''" alt="Gallery 4"></div>
      <div class="gallery__item rv" data-lightbox><img src="''' + gallery_imgs[4] + '''" alt="Gallery 5"></div>
      <div class="gallery__item rv" data-lightbox><img src="''' + gallery_imgs[5] + '''" alt="Gallery 6"></div>
    </div>
  </div>
</section>
    ...
    <div class="calc">
      ...
      <div class="calc__grid rv">
        <div class="calc__field">
          <label class="calc__label">Формат</label>
        ...
        <div class="calc__field">
          <label class="calc__label">Количество гостей</label>
        ...
        <div class="calc__field">
          <label class="calc__label">Дата</label>
...
<section class="section section--dark" id="contact">
  <div class="container">
    ...
    <div class="contact__grid" style="margin-top:2rem">
      ...
      <form class="contact__form rv-right" id="contactForm">
        <input type="text" class="contact__input" placeholder="Ваше имя" required>
        <input type="tel" class="contact__input" placeholder="Телефон" required>
        <input type="email" class="contact__input" placeholder="Email">
        <textarea class="contact__textarea" placeholder="Расскажите о вашем мероприятии"></textarea>
        <button type="submit" class="btn btn--gold" style="-webkit-align-self:flex-start;align-self:flex-start">Отправить заявку</button>
      </form>
    </div>
  </div>
</section>
...
<footer class="footer">
  <div class="container">
    <div class="footer__inner">
      <div class="footer__copy">Nilov Catering 2024. Все права защищены.</div>
      <div class="footer__links">
        <a href="#philosophy">О компании</a>
...
<div class="lightbox" id="lightbox">
  ...
  <img class="lightbox__img" id="lightboxImg" src="" alt="Gallery image">
</div>

<!-- WHATSAPP FLOAT -->
<a href="https://wa.me/78121234567" class="wa-float" rel="noopener" aria-label="WhatsApp">
...
(function() {
  ...
  if (heroImg) {
    if (heroImg.complete) {
      ...
      heroImg.addEventListener('error', function() { this.classList.add('loaded'); });
    }
  }
  ...
  if (burger && mobileMenu) {
    ...
    var mLinks = mobileMenu.querySelectorAll('.mobile-menu__link');
