#!/usr/bin/env python3
"""
Build Nilov Catering v16 — EDITORIAL WORLD-CLASS
Major redesign: editorial layouts, asymmetric grids, signature moments,
visual storytelling, prominent trust signals, dramatic typography
...
OUT = BASE / "download" / "nilov_catering_v16.html"
...
:root{{
--bg:#FAF8F5;--bg2:#F0EBE3;--dk:#0C0B09;--dk2:#161412;
...
/* PROGRESS */
.prog{{position:fixed;top:0;left:0;height:2px;background:linear-gradient(90deg,var(--g),var(--gl));z-index:1100;width:0%}}
...
.hero-ov2{{position:absolute;inset:0;background:radial-gradient(ellipse at 20% 80%,rgba(184,149,90,.08) 0%,transparent 50%)}}
...
.hero h1{{font-size:clamp(48px,12vw,130px);color:#fff;line-height:.85;letter-spacing:-.05em;font-weight:300;margin-bottom:28px;max-width:1000px}}
...
/* ═══ TRUST STRIP ═══ */
.tstrip{{background:var(--dk);padding:40px 28px;display:flex;justify-content:center;flex-wrap:wrap;gap:0;border-top:1px solid rgba(255,255,255,.03)}}
.ts{{text-align:center;padding:0 36px;border-right:1px solid rgba(255,255,255,.05)}}
.ts:last-child{{border-right:none}}
@media(max-width:640px){{.ts{{padding:0 20px 16px;border-right:none;border-bottom:1px solid rgba(255,255,255,.03)}}.ts:last-child{{border-bottom:none;padding-bottom:0}}}}
.ts-n{{font-family:var(--sf);font-size:clamp(32px,5vw,52px);color:var(--g);font-weight:300;line-height:1;letter-spacing:-.02em}}
.ts-l{{font-size:10px;color:rgba(255,255,255,.3);letter-spacing:2px;text-transform:uppercase;margin-top:6px}}
...
/* ═══ FORMATS — EDITORIAL ASYMMETRIC ═══ */
.fmt-sec{{padding:100px 28px;max-width:1200px;margin:0 auto}}
@media(min-width:768px){{.fmt-sec{{padding:140px 48px}}}}
.fmt-l{{display:grid;grid-template-columns:1fr;gap:48px}}
@media(min-width:768px){{.fmt-l{{grid-template-columns:1fr 1fr;gap:40px 56px}}}}
.fmt-l-item{{display:grid;grid-template-columns:1fr;gap:20px;cursor:pointer;padding:28px;border-radius:20px;border:1px solid var(--bd);transition:all .5s var(--eo);background:#fff}}
@media(min-width:768px){{.fmt-l-item{{grid-template-columns:200px 1fr;padding:32px;gap:28px}}}}
.fmt-l-item:hover{{box-shadow:0 16px 48px rgba(0,0,0,.06);transform:translateY(-4px);border-color:transparent}}
.fmt-l-item.wide{{grid-column:1/-1}}
@media(min-width:768px){{.fmt-l-item.wide{{grid-column:1/-1;grid-template-columns:280px 1fr}}}}
.fmt-l-img{{width:100%;aspect-ratio:4/3;object-fit:cover;border-radius:14px;transition:transform .7s var(--eo)}}
@media(min-width:768px){{.fmt-l-img{{aspect-ratio:3/4;height:100%}}}}
.fmt-l-item:hover .fmt-l-img{{transform:scale(1.04)}}
.fmt-l-icon{{width:40px;height:40px;border-radius:10px;background:var(--gd);display:flex;align-items:center;justify-content:center;margin-bottom:12px}}
.fmt-l-icon svg{{width:20px;height:20px;stroke:var(--a);fill:none;stroke-width:1.5;stroke-linecap:round;stroke-linejoin:round}}
.fmt-l-name{{font-family:var(--sf);font-size:28px;font-weight:500;margin-bottom:4px}}
.fmt-l-price{{font-size:17px;color:var(--g);font-weight:600;margin-bottom:12px}}
.fmt-l-desc{{font-size:14px;color:var(--tm);line-height:1.7;margin-bottom:16px}}
.fmt-l-cta{{display:inline-flex;align-items:center;gap:6px;font-size:11px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:var(--a);transition:gap .3s}}
.fmt-l-item:hover .fmt-l-cta{{gap:12px}}
...
.bleed-ov{{position:absolute;inset:0;background:linear-gradient(160deg,rgba(12,11,9,.9) 0%,rgba(12,11,9,.55) 100%)}}
...
.cbtn{{display:inline-flex;align-items:center;gap:8px;padding:15px 32px;background:var(--a);color:#fff;border:none;border-radius:10px;font-size:13px;font-weight:700;cursor:pointer;letter-spacing:.5px;text-transform:uppercase;transition:background .3s,transform .2s var(--es)}}
.cbtn:hover{{background:var(--ad);transform:scale(1.03)}}
...
.as-n{{font-family:var(--sf);font-size:44px;font-weight:300;color:var(--ad);line-height:1;letter-spacing:-.02em}}
...
.coc-i{{width:46px;height:46px;border-radius:12px;display:flex;align-items:center;justify-content:center;flex-shrink:0}}
.coc-i svg{{width:22px;height:22px;fill:currentColor}}
.coc-i.wa{{background:rgba(37,211,102,.08);color:var(--wa)}}
.coc-i.ph{{background:var(--gd);color:var(--a)}}
.coc-i.em{{background:var(--gd);color:var(--a)}}
...
.cof-b{{padding:15px 28px;background:var(--a);color:#fff;border:none;border-radius:10px;font-size:13px;font-weight:700;cursor:pointer;letter-spacing:.5px;text-transform:uppercase;transition:all .3s}}
.cof-b:hover{{background:var(--ad);transform:scale(1.02)}}
...
<a href="#faq" onclick="cMob()">Вопросы</a>
...
<!-- TRUST STRIP -->
<div class="tstrip">
<div class="ts"><div class="ts-n">19</div><div class="ts-l">лет опыта</div></div>
<div class="ts"><div class="ts-n">2 500+</div><div class="ts-l">мероприятий</div></div>
<div class="ts"><div class="ts-n">HACCP</div><div class="ts-l">стандарт</div></div>
<div class="ts"><div class="ts-n">98%</div><div class="ts-l">довольных</div></div>
</div>

<!-- MARQUEE -->
<div class="mq">
<div class="mq-t">
<span class="mq-i">Фуршеты<span class="d"></span></span>
<span class="mq-i">Банкеты<span class="d"></span></span>
<span class="mq-i">Кофе-брейки<span class="d"></span></span>
<span class="mq-i">Свадьбы<span class="d"></span></span>
<span class="mq-i">Корпоративы<span class="d"></span></span>
<span class="mq-i">Дегустации<span class="d"></span></span>
<span class="mq-i">Фуршеты<span class="d"></span></span>
<span class="mq-i">Банкеты<span class="d"></span></span>
<span class="mq-i">Кофе-брейки<span class="d"></span></span>
<span class="mq-i">Свадьбы<span class="d"></span></span>
...
<p class="ssub rv rv-up d1">Три основных формата — от лёгкого фуршета до торжественного банкета. Каждый адаптируем под вашу задачу.</p>
<div class="fmt-l">
<div class="fmt-l-item rv rv-up d1" onclick="selFmt('furshet')">
<img src="{IMG["furshet"]}" alt="Фуршет" class="fmt-l-img">
<div>
<div class="fmt-l-icon"><svg viewBox="0 0 24 24"><path d="M12 2L2 7l10 5 10-5-10-5z"/><path d="M2 17l10 5 10-5"/><path d="M2 12l10 5 10-5"/></svg></div>
<div class="fmt-l-name">Фуршет</div>
<div class="fmt-l-price">от 2 450 ₽ / гость</div>
<div class="fmt-l-desc">Канапе, брускетты, тарталетки и горячие закуски. Идеально для приёма, открытия, корпоратива.</div>
<div class="fmt-l-cta">Рассчитать →</div>
</div>
</div>
<div class="fmt-l-item rv rv-up d2" onclick="selFmt('banket')">
<img src="{IMG["banket"]}" alt="Банкет" class="fmt-l-img">
<div>
<div class="fmt-l-icon"><svg viewBox="0 0 24 24"><path d="M3 20h18"/><path d="M5 20V12a2 2 0 012-2h10a2 2 0 012 2v8"/><path d="M8 10V7a4 4 0 018 0v3"/><circle cx="12" cy="15" r="2"/></svg></div>
<div class="fmt-l-name">Банкет</div>
<div class="fmt-l-price">от 4 470 ₽ / гость</div>
<div class="fmt-l-desc">Полноценный ужин с обслуживанием официантов. Сервировка, посуда, текстиль включены.</div>
<div class="fmt-l-cta">Рассчитать →</div>
</div>
</div>
<div class="fmt-l-item wide rv rv-up d3" onclick="selFmt('coffee')">
<img src="{IMG["coffee"]}" alt="Кофе-брейк" class="fmt-l-img">
<div>
<div class="fmt-l-icon"><svg viewBox="0 0 24 24"><path d="M17 8h1a4 4 0 010 8h-1"/><path d="M3 8h14v9a4 4 0 01-4 4H7a4 4 0 01-4-4V8z"/><line x1="6" y1="2" x2="6" y2="4"/><line x1="10" y1="2" x2="10" y2="4"/><line x1="14" y1="2" x2="14" y2="4"/></svg></div>
<div class="fmt-l-name">Кофе-брейк</div>
<div class="fmt-l-price">от 950 ₽ / гость</div>
<div class="fmt-l-desc">Кофе, чай, выпечка и лёгкие закуски для конференций, семинаров и деловых встреч.</div>
<div class="fmt-l-cta">Рассчитать →</div>
</div>
</div>
</div>
...
<div class="cgr">
<label class="cgl">Формат</label>
<select class="csel" id="cFmt">
...
<div class="cgr">
<label class="cgl">Количество гостей</label>
<input type="range" class="crn" id="cGst" min="10" max="300" value="50" step="5">
<div class="crn-info"><span>10</span><span class="crn-val"><strong id="cGstV">50</strong> чел.</span><span>300</span></div>
</div>
<div class="cgr">
<label class="cgl">Дополнительно</label>
<select class="csel" id="cExt">
...
<div class="cres">
<div class="cres-lbl">Приблизительная стоимость</div>
<div class="cres-p" id="cPrice">122 500 ₽</div>
<div class="cres-n">Финальная стоимость зависит от меню и пожеланий</div>
<a href="{WA}" class="cbtn" target="_blank" rel="noopener">
<svg viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
...
<p class="ssub rv rv-up d1">Реальные отзывы с проверенных площадок. Нам доверяют крупнейшие компании.</p>
<div class="tg">
<div class="tc rv rv-up d1"><div class="tc-s">★★★★★</div><div class="tc-tx">Корпоратив на 200 человек прошёл безупречно. Идеальный тайминг, потрясающая подача.</div><div class="tc-a">Анна Соколова</div><div class="tc-e">Корпоратив, 200 гостей</div></div>
<div class="tc rv rv-up d2"><div class="tc-s">★★★★★</div><div class="tc-tx">Свадьба мечты благодаря Nilov Catering! Меню подобрали с учётом всех пожеланий.</div><div class="tc-a">Екатерина и Дмитрий</div><div class="tc-e">Свадьба, 120 гостей</div></div>
<div class="tc rv rv-up d3"><div class="tc-s">★★★★★</div><div class="tc-tx">Третий год сотрудничаем — кофе-брейки всегда на высоте. Пунктуальная доставка.</div><div class="tc-a">Игорь Петров</div><div class="tc-e">Кофе-брейки, ежемесячно</div></div>
<div class="tc rv rv-up d4"><div class="tc-s">★★★★★</div><div class="tc-tx">Дегустация убедила сразу — качество ингредиентов на уровне хорошего ресторана.</div><div class="tc-a">Марина Климова</div><div class="tc-e">Фуршет, 80 гостей</div></div>
</div>
</section>
...
<p class="ssub rv rv-up d1">Каждое блюдо — маленькое произведение. Готовим из свежих продуктов, подаём с эстетикой ресторанного уровня.</p>
<div class="mas">
...
<div class="fi rv rv-up d1"><div class="fq" onclick="tF(this)">Какое минимальное количество гостей?<span class="fp">+</span></div><div class="fa">Фуршет — от 20 гостей, банкет — от 15, кофе-брейк — от 10.</div></div>
<div class="fi rv rv-up d2"><div class="fq" onclick="tF(this)">Выезжаете ли за пределы КАД?<span class="fp">+</span></div><div class="fa">Основная зона — Санкт-Петербург в пределах КАД. Выезд обсуждается индивидуально.</div></div>
<div class="fi rv rv-up d3"><div class="fq" onclick="tF(this)">Можно ли провести дегустацию?<span class="fp">+</span></div><div class="fa">Да, проводим бесплатную дегустацию для заказов от 30 гостей.</div></div>
<div class="fi rv rv-up d4"><div class="fq" onclick="tF(this)">Что входит в стоимость?<span class="fp">+</span></div><div class="fa">Приготовление, доставка, сервировка, обслуживание, посуда, текстиль, уборка.</div></div>
<div class="fi rv rv-up d5"><div class="fq" onclick="tF(this)">За сколько дней бронировать?<span class="fp">+</span></div><div class="fa">Рекомендуем за 2–3 недели. В сезон свадеб — за месяц.</div></div>
<div class="fi rv rv-up d6"><div class="fq" onclick="tF(this)">Есть ли блюда для веганов?<span class="fp">+</span></div><div class="fa">Веганские, безглютеновые, безлактозные блюда — укажите при заказе.</div></div>
</div>
</section>
...
<p class="ssub rv rv-up d1">Напишите или позвоните — ответим в течение 15 минут.</p>
<div class="cog">
<div class="com rv rv-l d2">
<a href="{WA}" class="coc" target="_blank" rel="noopener">
<div class="coc-i wa"><svg viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg></div>
<div><div class="coc-l">WhatsApp</div><div class="coc-v">+7 (911) 941-72-05</div></div>
</a>
<a href="{TEL}" class="coc">
<div class="coc-i ph"><svg viewBox="0 0 24 24"><path d="M6.62 10.79a15.053 15.053 0 006.59 6.59l2.2-2.2a1.003 1.003 0 011.01-.24c1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.1.31.03.66-.25 1.02l-2.2 2.2z"/></svg></div>
...
<div class="coc-i em"><svg viewBox="0 0 24 24"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/></svg></div>
...
<!-- FOOTER -->
<footer class="ft">
<div class="ft-t"><strong>Interfood Catering</strong> · Санкт-Петербург<br>
