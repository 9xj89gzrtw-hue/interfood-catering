#!/usr/bin/env python3
"""
NILOV CATERING v12 — Beat The Best

Fixes from VLM critique of v11:
- Tighter layout, less dead space
- Unified warm palette: deep burgundy, warm cream, gold
- Bigger, bolder typography with real hierarchy
- Hero: full-bleed, dramatic, with better overlay
- Format cards: more visual, less text
- Gallery: tighter mosaic with better sizing
- Contact: warmer, more inviting
- WhatsApp button: subtle, not neon
- Scroll reveal animations (Telegram-safe)
"""
...
OUT = Path('/home/z/my-project/download/nilov_catering_v12.html')
...
def compress_b64(b64_path, max_width=1200, quality=80):
...
def build():
    ...
    for key, fname in SELECTED.items():
        ...
        b64 = compress_b64(src, max_width=max_w, quality=80)
        imgs[key] = b64
...
<meta name="theme-color" content="#2B2028">
...
:root{{
  ...
  --cream-warm:#EDE5DA;
  --burgundy:#8C3540;
  --burgundy-hover:#A04450;
  --gold:#C4993D;
  --gold-soft:rgba(196,153,61,0.15);
  --text:#2B2028;
  --text-mid:#7B6E76;
  --text-light:#A89B9F;
  --wa:#34A853;
...
.nav-center{{display:none;align-items:center;gap:28px}}
...
.nav.solid .nav-btn:hover{{background:var(--burgundy-hover)}}
...
.hero-ov{{
  position:absolute;inset:0;
  background:
    radial-gradient(ellipse 90% 80% at 30% 90%, rgba(43,32,40,0.95) 0%, transparent 60%),
    linear-gradient(180deg, rgba(43,32,40,0.15) 0%, rgba(43,32,40,0.35) 40%, rgba(43,32,40,0.8) 80%, rgba(43,32,40,1) 100%);
}}
.hero-content{{
...
@media(min-width:768px){{.hero-content{{padding:0 64px 120px;max-width:720px}}}}
...
.hero-cta:hover{{background:var(--burgundy-hover)}}
...
@media(min-width:960px){{.formats-grid{{grid-template-columns:1fr 1fr 1fr}}}}

.fmt{{
  position:relative;overflow:hidden;border-radius:6px;
  aspect-ratio:3/4;cursor:pointer;background:var(--dark);
}}
.fmt-bg{{
  position:absolute;inset:0;
...
.fmt::after{{
  ...
  background:linear-gradient(to top,rgba(43,32,40,0.92) 0%,rgba(43,32,40,0.25) 55%,rgba(43,32,40,0.04) 100%);
}}
.fmt-body{{
...
.fmt-name{{
  ...
  font-weight:400;line-height:1.12;margin-bottom:4px;
}}
.fmt-price{{
...
@media(min-width:768px){{.about-grid{{grid-template-columns:300px 1fr;gap:56px}}}}
.about-photo{{
  border-radius:6px;overflow:hidden;
  aspect-ratio:3/4;
  box-shadow:0 12px 40px rgba(43,32,40,0.12);
}}
...
.about-role{{
  font-size:12px;color:var(--burgundy);font-weight:700;
...
.about-stat-num{{
  font-family:var(--serif);
  font-size:34px;font-weight:400;color:var(--burgundy);
...
@media(min-width:768px){{.g-big{{grid-column:span 3;grid-row:span 2}}}}
.gallery-grid img{{
...
.quote{{
  background:var(--dark);
  padding:80px 28px;text-align:center;color:#fff;
}}
@media(min-width:768px){{.quote{{padding:100px 48px}}}}
.quote blockquote{{
  font-family:var(--serif);
  font-size:clamp(20px,3vw,32px);
...
.faq-q::after{{
  content:"+";font-size:18px;font-weight:300;color:var(--text-light);
  flex-shrink:0;
}}
...
.cm-icon.wa{{background:rgba(52,168,83,0.08)}}
.cm-icon.wa svg{{fill:var(--wa)}}
.cm-icon.ph{{background:rgba(140,53,64,0.06)}}
.cm-icon.ph svg{{fill:var(--burgundy)}}
.cm-icon.em{{background:rgba(140,53,64,0.06)}}
.cm-icon.em svg{{fill:var(--burgundy)}}
.cm-label{{font-size:10px;color:var(--text-light);font-weight:600;letter-spacing:1.5px;text-transform:uppercase}}
.cm-value{{font-size:16px;font-weight:500}}
...
.fsb:hover{{background:var(--burgundy-hover)}}
...
.foot-brand{{font-family:var(--serif);font-size:16px;color:#fff;margin-bottom:2px}}
.foot-copy{{font-size:12px}}
.foot-links{{display:flex;gap:20px;flex-wrap:wrap;justify-content:center}}
.foot-links a{{font-size:12px;color:rgba(255,255,255,0.45);transition:color .2s}}
...
.wa-f svg{{width:24px;height:24px;fill:currentColor}}
...
<section class="hero">
  ...
  <div class="hero-content">
    <div class="hero-tag">Санкт-Петербург · с 2007</div>
...
<!-- TRUST -->
<section class="trust">
  <span><strong>19 лет</strong> на рынке</span>
  <span><strong>2 500+</strong> мероприятий</span>
  <span><strong>HACCP</strong> сертификация</span>
  <span><strong>12</strong> форматов</span>
</section>
...
<section class="sec" id="formats">
  <div class="sec-inner">
    <div class="rv">
      <div class="sec-tag">Форматы</div>
      <h2 class="sec-title">Подберём формат</h2>
      <p class="sec-lead">От кофе-брейка на 10 человек до банкета на 500 гостей.</p>
    </div>
    <div class="formats-grid rv">
      <div class="fmt">
...
<!-- GALLERY -->
<section id="gallery">
  <div class="sec-inner rv" style="padding:0 28px 40px">
    <div class="sec-tag">Галерея</div>
    <h2 class="sec-title">Наши блюда</h2>
  </div>
  <div class="gallery-grid rv" style="max-width:1400px;margin:0 auto">
    <img class="g-big" src="data:image/jpeg;base64,{imgs['gallery_1']}" alt="">
    ...
    <img class="g-big" src="data:image/jpeg;base64,{imgs['gallery_6']}" alt="">
...
<!-- CONTACT -->
<section class="sec" id="contact" style="background:var(--cream-light)">
  <div class="sec-inner">
...
// Reveal (Telegram-safe, no IntersectionObserver)
(function(){{
  var els=document.querySelectorAll('.rv');
  function check(){{
    var h=window.innerHeight;
    for(var i=0;i<els.length;i++){{
      if(els[i].getBoundingClientRect().top<h-40)els[i].classList.add('v');
    }}
  }}
