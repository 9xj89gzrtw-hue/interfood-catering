...
/* ─── Feature Cards ─── */
const FEATURES = [
  { icon: "✦", title: "Авторское меню", desc: "Шеф-повар создаёт уникальные блюда, которые отражают вашу историю любви и подчёркивают стиль торжества. Каждое блюдо разрабатывается индивидуально — от приветственного аперитива до свадебного торта. Перед заказом мы приглашаем вас на персональную дегустацию, чтобы каждый вкус был безупречным." },
  { icon: "◈", title: "Персональный менеджер", desc: "Ваш личный кейтеринг-консьерж доступен 24/7 на всех этапах подготовки и в день свадьбы. Он координирует работу с фотографом, флористом, ведущим и другими подрядчиками, контролирует таймлайн и решает любые вопросы. Вам останется только наслаждаться своим днём." },
